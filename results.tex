...
\add{\bf \st{Before we go into the results, we need to provide some information 
...
\begin{table}[t]
    \centering
    \caption{\makenote{add additional columns covering dimensionality (real, 
    categorical, total \# of categories)}
    Characteristics of datasets used in the analysis.  $N$ refers to 
    ...
    set of parameters with the \emph{over} subindex, while rank 
    transformation and categorical analysis use  the set of parameters with 
    the \emph{sub} subindex.
    \label{tab:data}
    }
    \bigskip
...
We compared our four proposed scores against three canonical novelty detection 
    ...
    performance tables seen in this section. \add{\bf Here you need a description
    of what was done in each of the three sections that follow. My understanding is
    that you first used a purely categorical scoring. Then used mixed scoring
    with thresholding of the continuous variables. Finally you used the ranked 
