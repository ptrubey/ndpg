...
The complete field of anomaly detection is vast.  However, most methods can be 
    ...
    \add{\bf We need references in this paragraph}
...
{\bf Clustering Methods} group data into clusters of similar observations.
    ...
    tend to stand apart from non-anomalous data.\add{\bf We need references here as well.}
...
The application of multivariate extreme value theory to anomaly detection is based on the 
...
A central result of multivariate EVT is that, conditional on an observation being 
    ...
    % employs a theoretical argument from EVT to establish a threshold
    ...
    % For this reason, in this paper we develop approaches that account for the 
    % dependence structure of the tails of the distribution.




  % \add{\bf We need a paragraph where we summarize what we are proposing, emphasizing the 
  % challenges of the problem, and how we solved them, to argue why this is a valuable contribution.}
   
% \add{\bf Here we need a roadmap to the paper. }
