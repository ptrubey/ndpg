...
Suppose $C$ is a vector of $M$ random categorical variables.  Then $\bm{C}_{m}$ 
    ...
    established as \add{\bf This paragraph needs more clarity. In particular, the 
    notation needs to be consisten: Bold face for vectors; Capital letters for random
    variables. Thus a random vector is bold face capitalized. Then there is the story of
    the Dirichlet-multinomial that needs more explanation. We say that $C_m$ (or is it 
    $\bm{C}_m$?) is multinomial. Then we say that, to deal with overdispersion, we consider
    a Dirichlet-Multinomial, as a generalization of the beta-binomial. Then you write,
    explicitly, how the $PG_1$ is brought in. Finally, you bring the Pitman Yor mixture
    and comment that we have proposed a very flexible model for categorical data.}
    \begin{equation}
