...
We employ a univariate representation variation on Silverman's rule of thumb\makenote{find citation}
    ...
    This then requires the estimation of $\hat{\sigma}$, which must be calculated from pairwise distances.

\[ \text{E}\left\lVert \bm{X}_j - \bm{X}_k\rVert_2\right] = 2\text{Var}(\bm{X}) \]
where $\bm{X}_j$ and $\bm{X}_k$ are both replicates drawn from $\bm{X}\mid\bm{x}$.  Thus,
\[
